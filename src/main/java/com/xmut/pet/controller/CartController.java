...
public class CartController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "goodsIds", dataType = "Integer", paramType = "query", value = "购物车id", required = true),
    })
    @RequestMapping(method = RequestMethod.POST, value = "/delete")
    public Result<Cart> delete(String goodsIds) {
        Result<Cart> result = new Result();
        if (cartService.delete(goodsIds)) {
            result.success("删除成功");
        }
        return result;
    }
    ...
    public Result<List<Store>> pageCart() {
        Cart cart = new Cart();
    ...
    public Result<Cart> updateNum(Integer goodsId, Integer num) {
        Result<Cart> result = new Result<>();
        if (cartService.updateNum(goodsId, num)) {
            result.success("修改成功");
        } else {
            result.fail("库存不足");
        }
    ...
    public Result updateSelected(Integer goodsId, boolean selected) {
        Result result = new Result();
        if (cartService.updateSelected(goodsId, selected)) {
            result.success("修改成功");
        } else {
            result.setData("修改失败");
        }
